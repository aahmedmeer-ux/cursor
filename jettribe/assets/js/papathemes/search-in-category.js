...
export default class SearchInCategory {
    ...
    onStateChange() {
        ...
        const locationUrl = urlUtils.getUrl(); // papathemes: infinite scroll + search in category compatibility
        ...
        let requestOptions = {
            ...
            config: {
                ...
                shop_by_brand: {
                    limit: this.options.context.sidebar_shop_by_brand,
                },
            },
        };
        ...
        api.getPage(searchUrl, requestOptions, (err, content) => {
            ...
            if (content && this.options.searchCallback) {
                this.options.searchCallback(content, locationUrl); // papathemes: infinite scroll + search in category compatibility
            }
    ...
    updateViewFacetedSearch() {
        ...
        $(facetedSearch.options.blockerSelector).addClass('loading');
        ...
        api.getPage(requestUrl, requestOptions, (err, content) => { // Supermarket MOD
            ...
            $(facetedSearch.options.blockerSelector).removeClass('loading');
            this.$form.removeClass('loading');
            ...
            if (content?.sidebar) {
                ...
                const existingAccordionTargetId = $(facetedSearch.options.accordionToggleSelector)
                    .get()
                    .map(el => $(el).data('collapsibleInstance')?.targetId)
                    .filter(id => id);
                ...
                $(content.sidebar).find(facetedSearch.options.accordionToggleSelector).each((i, el) => {
                    const $el = $(el);
                    const id = $el.data('collapsible').replace('#', '');
            ...
            const layout = this.options.context.categorypage_layout;
            if (labelSearch && layout === 'fullwidth') {
