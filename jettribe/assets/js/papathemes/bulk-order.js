...
export class BulkOrder {
    ...
    onQuantityChange(event) {
        ...
        this.updateQtySatisfyMinMax($input);
    ...
    updateQtySatisfyMinMax($input) {
        ...
        const newQty = String(qty <= 0 && allowZero ? 0 : minmax(qty));
    ...
    updateQtyInCart() {
        // Stop if there is element contains qty in cart not visible
        if (this.$scope.find('[data-bulkorder-cart-qty-id]:visible').length === 0) {
            return;
        }
