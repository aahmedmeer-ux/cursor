...
import { fixFormElementUniqueIds } from '../../papathemes/utils'; // papathemes-dinosaur
...
export default class CartItemDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        fixFormElementUniqueIds($form);
        this.initUploadFile();

        // papathemes-dinosaur: add product design tool if product options have pd_modifier_name
        if (this.context.pd_modifier_name && $scope.find(`[data-pd-option-name="${this.context.pd_modifier_name}"]`).length > 0) {
            $scope.find('[data-design-tool-wrapper]').show();
            const productId = this.context.productForChangeId;
            this.designTool = new ProductDesignTool({
                $scope, $form, productId, context,
            });
        }
    }

    initUploadFile() {
        $('#CartEditProductFieldsForm [data-product-attribute="input-file"] button', this.$scope).on('click', (e) => {
            e.preventDefault();
            $(e.currentTarget).next().trigger('click');
        });

        $('#CartEditProductFieldsForm input[type=file]', this.$scope).on('change', (e) => {
    ...
    setProductVariant() {
        ...
        const view = $('.modal-header-title._configureProduct');
